use std::fs;
mod measure_window;
use measure_window::MeasureWindow;
...
fn main() {
  ...
  for n in 1..values.len() - (WINDOW_SIZE - 1) {
    ...
    let previous_window = MeasureWindow::from_slice(&values[previous_window_starting_index..previous_window_ending_index]);

    let current_window_starting_index = n;
    let current_window_ending_index = current_window_starting_index + WINDOW_SIZE;
    let current_window = MeasureWindow::from_slice(&values[current_window_starting_index..current_window_ending_index]);

    if current_window.sum() > previous_window.sum() {
      increments += 1;
    }
  }
